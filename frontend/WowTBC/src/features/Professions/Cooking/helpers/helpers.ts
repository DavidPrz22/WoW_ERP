...
export function calculateGrandTotals(groupsData: Array<{ type: string; items: Array<{ name: string; AHPrice: number; craftingCost: number }> }>, quantities: Record<string, number>) {
  let totalCost = 0;
  let totalProfit = 0;
  ...
  for (const group of groupsData) {
    for (const item of group.items) {
      ...
      totalCost += item.craftingCost * qty;
      totalProfit += profitPerItem * qty;
    }
  }

  return { cost: totalCost, profit: totalProfit };
}
